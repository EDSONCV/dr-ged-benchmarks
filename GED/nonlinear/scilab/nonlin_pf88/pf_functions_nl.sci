...
function y = dg1(x)
//flowsheet_residuals is it's own file
ytmp = diffcode_jacobian(pf88_residuals,x)';
//disp('inside dg')
//pause
for i = 1: nnzjac; 
...
//    pause
    if sum(abs(lambda)) <> 0           then
        ...
        //flowsheet_residuals is it's own file
        ytmpconstr = diffcode_hessian(pf88_residuals,x);
...
function [nc, nv, nnzjac, nnz_hess, sparse_dg, sparse_dh, lower, upper, var_lin_type, constr_lin_type, constr_lhs, constr_rhs]  = pf_struct_nl(meas_full)
// Data Reconciliation Benchmark Problems From Literature Review
// Author: Edson Cordeiro do Valle
// Contact - edsoncv@{gmail.com}{vrtech.com.br}
// Skype: edson.cv
...
//    meas_full    measurements (or estimates, in case of unmeasured stream)
...
jactst = diffcode_jacobian(pf88_residuals,xlocal)';
...
hess_constr_tst = diffcode_hessian(pf88_residuals,xlocal);
...
// the constraints DO NOT HAVE lower and upper bound = 0
constr_lhs = [255.8 -111.2 33.57 0 0 126.6]';
constr_rhs = [255.8 -111.2 33.57 0 0 126.6]';
