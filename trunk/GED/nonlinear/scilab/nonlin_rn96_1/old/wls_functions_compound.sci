...
function f = objfun ( x )
    ...
    f = obj_full_compounds(x);

endfunction

function c = confun(x)
//compound_residuals is it's own file
	c =  compound_residuals(x,jac,ncomp);
...
// statistics ).
//    gf = derivative(obj_full_compounds, x, order = 4)';
    gf = diffcode_jacobian(obj_full_compounds,x);
// If one wants to use the classical weighted 
// least squares approach, the evaluation of the objective function gradient
// using the function bellow is less lime consuming    
//gf = zeros(length(x),1);
//gf(red) = 2*((x(red) - xmfull(red))./(var(red)));
...
//(eg. in case the user want's to use a objective function form robust
// statistics ).
//    H = diffcode_hessian(obj_full_compounds,x);
    [J,H] = derivative(obj_full_compounds, x, H_form = "hypermat", order = 4);
// If one wants to use the classical weighted 
// least squares approach, the evaluation of the hessian using the function
// bellow is less time consuming    
// notice that, since Hessian is constant, setting this option in ipopt will
// result in a much faster optimziation, since Hessian will be evaluated only one
// time.
//    H = zeros(nv,nv);
//    varint = zeros(nv,1);
////    varint(red) = 2*ones(length(red),1)./var(red);
//    H(1:nv,1:nv) = diag(varint);
...
//    ytmp = diffcode_jacobian(list(compound_residuals,jac,ncomp),x)';
    ytmp =  derivative(list(compound_residuals,jac,ncomp), x, order = 2);
    for i = 1: nnzjac; 
...
//        ytmpconstr = diffcode_hessian_par(flowsheet_residuals,x,14*ndata);
//         ytmpconstr = diffcode_hessian(flowsheet_residuals,x);
        [J,ytmpconstr] = derivative(list(compound_residuals,jac,ncomp), x, order = 4);

        for i = 1: nc; 
            if lambda(i) <> 0 then
//                ysum = ysum + lambda(i)*ytmpconstr(:,:,i); 
                ytmp = lambda(i)*ytmpconstr(i,:); 
...
function [nc, nv, nnzjac, nnz_hess, sparse_dg, sparse_dh, lower, upper, var_lin_type, constr_lin_type, constr_lhs, constr_rhs]  = wls_structure_compound(jac,ncompounds, totalFlowMeasured,compoundMeasured)
// Data Reconciliation Benchmark Problems From Literature Review
// Author: Edson Cordeiro do Valle
// Contact - edsoncv@{gmail.com}{vrtech.com.br}
// Skype: edson.cv
